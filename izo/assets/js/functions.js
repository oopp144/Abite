...
izo.navigation = {
	init: function() {
		...
		button.addEventListener( 'click', function() {
			...
			var menuBarClone 	= header.getElementsByClassName( 'menu' )[0].cloneNode( true );
			menuBarClone.setAttribute( 'id', 'mobile-menu' );
			var opening			= siteNavigation.getAttribute('data-open');
			
			var clone = document.getElementById( 'mobile-menu' );
			menuBarClone.classList.add( opening );

			if ( menuBarClone.classList.contains( 'op-fullscreen' ) || menuBarClone.classList.contains( 'op-slide' ) ) {
				var closeButton = document.createElement('i');
				closeButton.classList.add( 'icon-cancel' );

				menuBarClone.appendChild( closeButton );
			...
			var focusableEls = menuBarClone.querySelectorAll('a[href]:not([disabled]), input[type="search"]:not([disabled])'),
				firstFocusableEl = focusableEls[0];  
				lastFocusableEl = focusableEls[focusableEls.length - 1];
				KEYCODE_TAB = 9;

			menuBarClone.addEventListener('keydown', function(e) {
			...
			//Append + for submenus
			const linksWithChildren = menuBarClone.querySelectorAll( '.menu-item-has-children > a, .page_item_has_children > a' );
			...
			//Toggle submenus
			const submenuToggles 	= menuBarClone.querySelectorAll( '.submenu-toggle' );
			...
			//Append the mobile menu
			if ( siteNavigation.classList.contains( 'toggled' ) ) {
				header.getElementsByClassName( 'bottom-header-bar' )[0].appendChild( menuBarClone );
			} else {
				clone.remove();
			}
